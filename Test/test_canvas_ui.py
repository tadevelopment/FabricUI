...
canvas_ref_images_dir = os.path.join(base_dir, "Native", "FabricUI", "Test", "UI")   
canvas_out_images_dir = os.path.join(base_dir, "build", os.environ['FABRIC_BUILD_OS'], os.environ['FABRIC_BUILD_ARCH'], os.environ['FABRIC_BUILD_TYPE'], "Native", "FabricUI", "Test", "UI")
canvas_tests_dir      = os.path.join(base_dir, "Native", "FabricUI", "Test", "UI")   
...
def comparison(canvas_win, dir, test, replace_ref_image = False):
  ...
  test_out_images_dir = os.path.join(canvas_out_images_dir, dir)
  image_filepath = os.path.join(test_out_images_dir, test)
  image_ref_filepath = os.path.join(canvas_ref_images_dir, dir, test)
  canvas_win.onNewGraph(skip_save=True)  
  canvas_win.loadGraph(canvas_filepath + os.extsep + "canvas")
...
for test in canvas_tests:  
  canvas_test = 'def test_' + test[1] + '(canvas_win, replace_ref_image = False): comparison(canvas_win, "' + test[0] + '", "' + test[1] + '", replace_ref_image)'
  exec(canvas_test)
...
if __name__ == '__main__':  
  ...
  parser.add_option("-r", "--replace", dest="replace_ref_image", help="Replace the given test image", default=False)
  options, args = parser.parse_args()
  if options.test_name:
    try:      
      update_ref_image = options.replace_ref_image
      exec("test_" + os.path.splitext(options.test_name)[0] + "(canvas_win_instance, " + str(update_ref_image) + ")")
