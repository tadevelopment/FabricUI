  ...
  )
{
  ...
  try
  {
    ...
    {
      FabricUI::ValueEditor::BaseModelItem* changingChild = m_modelRoot->getChild( name, false );
  ...
  )
{
  ...
  try
  {
    if ( FabricUI::ValueEditor::BaseModelItem *changingChild =
...
void DFGVEEditorOwner::onOutputsChanged()
{
  ...
  for (; itr != end; itr++)
  {
    ...
    if ( FabricUI::ValueEditor::ItemMetadata *childItemMetadata = childModelItem->getMetadata() )
    {
      FTL::CStrRef vePortType =
        childItemMetadata->getString( FabricUI::ValueEditor::ItemMetadata::VEPortTypeKey.c_str() );
...
void DFGVEEditorOwner::onBindingArgRemoved( unsigned index, FTL::CStrRef name )
{
  ...
  if ( m_modelRoot->isBinding() )
  {
    ...
    FabricUI::ValueEditor::BaseModelItem* removedChild = m_modelRoot->getChild( name, false );
  ...
  )
{
  ...
  if ( m_modelRoot->isNode() )
  {
    BindingModelItem *bindingModelItem =
      static_cast<BindingModelItem *>( m_modelRoot );
    FabricUI::ValueEditor::BaseModelItem* removedChild = m_modelRoot->getChild( portName, false );
    if ( removedChild != NULL )
    {
      emit modelItemRemoved( removedChild );
  ...
  )
{
  ...
  FabricUI::ValueEditor::BaseModelItem* changingChild = m_modelRoot->getChild( portName, false );
  ...
  )
{
  ...
  if ( m_modelRoot->isNode() )
  {
    ...
    {
      if ( FabricUI::ValueEditor::BaseModelItem* destChild =
        m_modelRoot->getChild( portName, false ) )
