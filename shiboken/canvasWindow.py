  ...
  def eventFilter(self, obj, event):
    ...
    if eventType == QtCore.QEvent.KeyPress:
      ...
      if keyEvent.key() != QtCore.Qt.Key_Tab:
        if not self.window.viewport: return False
...
class CanvasWindow(DFG.DFGMainWindow):
  ...
  def _initWindow(self):
    ...
    self.fpsTimer.timeout.connect(self.onUpdateFPS)
    self.fpsTimer.start()
  ...
  def _initDFG(self):
    ...
    tabSearchWidget.enabled.connect(self.onEnableShortCuts)
  ...
  def onPortManipulationRequested(self, portName):
    try:
      ...
      if portResolvedType == 'Xfo': pass
      elif portResolvedType == 'Mat44': value = value.toMat44('Mat44')
      elif portResolvedType == 'Vec3': value = value.tr
      elif portResolvedType == 'Quat': value = value.ori
      else:
  ...
  def __loadGraph(self, filePath):
    ...
    try:
      ...
      if(self.isCanvas): self.viewport.clearInlineDrawing()
      ...
      if len(camera_mat44) > 0 and len(camera_focalDistance) > 0:
        try:
          ...
          focalDistance = self.client.RT.constructRTValFromJSON('Float32', camera_focalDistance)
      ...
      if len(tl_current) > 0: self.timeLine.updateTime(int(tl_current), True)
      else: self.timeLine.updateTime(CanvasWindow.defaultFrameIn, True)
  ...
  def onSaveGraph(self):
    self.__saveGraph(False)

  def onSaveGraphAs(self):
    self.__saveGraph(True)

  def closeEvent(self, event):
    if not self.__checkUnsavedChanges():
      event.ignore()
      return
  ...
  def __checkUnsavedChanges(self):
    ...
    if binding.getVersion() != self.lastSavedBindingVersion:
      ...
      result = msgBox.exec_()
      if result == QtGui.QMessageBox.Discard: return True
      elif result == QtGui.QMessageBox.Cancel: return False
      else: return self.__saveGraph(False)
    return True
  ...
  def onFrameChanged(self, frame):
    ...
    try:
      ...
      if dfgExec.isExecPortResolvedType(self.timeLinePortIndex, "SInt32"):
        binding.setArgValue(self.timeLinePortPath, self.client.RT.types.SInt32(frame), False)
      elif dfgExec.isExecPortResolvedType(self.timeLinePortIndex, "UInt32"):
        binding.setArgValue(self.timeLinePortPath, self.client.RT.types.UInt32(frame), False)
      elif dfgExec.isExecPortResolvedType(self.timeLinePortIndex, "Float32"):
        binding.setArgValue(self.timeLinePortPath, self.client.RT.types.Float32(frame), False)
      elif dfgExec.isExecPortResolvedType(self.timeLinePortIndex, "Float64"):
        binding.setArgValue(self.timeLinePortPath, self.client.RT.types.Float64(frame), False)
    except Exception as e:
      self.dfgWidget.getDFGController().logError(str(e))

  def onUpdateFPS(self):
    pass
    #if not self.viewport:
    #    return
    #caption = str(round(self.viewport.fps(), 2)) + " FPS"
    #333self.fpsLabel.setText(caption)
  ...
  def autosave(self):
    ...
    if binding:
      ...
      if bindingVersion != self.lastAutosaveBindingVersion:
        ...
        if self.__performSave(binding, tmpAutosaveFilename):
  ...
  def onLoadGraph(self):
    ...
    lastPresetFolder = str(self.settings.value("mainWindow/lastPresetFolder"))
    filePath, _ = QtGui.QFileDialog.getOpenFileName(self, "Load graph", lastPresetFolder, "*.canvas")
  ...
  def __performSave(self, binding, filePath):
    ...
    try:
      ...
      graph.setMetadata("camera_mat44", str(mat44.getJSON()), False)
      graph.setMetadata("camera_focalDistance", str(focalDistance.getJSON()), False)
  ...
  def __saveGraph(self, saveAs):
    ...
    if len(filePath) == 0 or saveAs:
      ...
      filePath, _ = QtGui.QFileDialog.getSaveFileName(self, "Save graph", filePath, "*.canvas")
      if len(filePath) == 0:
        return False
      ...
      if self.__performSave(binding, filePath):
        self.evalContext.currentFilePath = filePath
  ...
  def onSetBlockCompilations(self, blockCompilations):
    dfgController = self.dfgWidget.getDFGController()
    dfgController.setBlockCompilations(blockCompilations)

  def onFileNameChanged(self, fileName):
    if not fileName: self.setWindowTitle(self.windowTitle)
    else: self.setWindowTitle(self.windowTitle + " - " + fileName)

  def onEnableShortCuts(self, enabled):
    if self.newGraphAction: self.newGraphAction.blockSignals(enabled)
    if self.loadGraphAction: self.loadGraphAction.blockSignals(enabled)
    if self.saveGraphAction: self.saveGraphAction.blockSignals(enabled)
    if self.saveGraphAsAction: self.saveGraphAsAction.blockSignals(enabled)
    if self.quitAction: self.quitAction.blockSignals(enabled)
    if self.manipAction: self.manipAction.blockSignals(enabled)
    if self.setGridVisibleAction: self.setGridVisibleAction.blockSignals(enabled)
    if self.resetCameraAction: self.resetCameraAction.blockSignals(enabled)
    if self.clearLogAction: self.clearLogAction.blockSignals(enabled)
    if self.blockCompilationsAction: self.blockCompilationsAction.blockSignals(enabled)
  ...
  def onAdditionalMenuActionsRequested(self, name, menu, prefix):
    if name == 'File':
      ...
      if prefix:
        ...
        self.blockCompilationsAction = QtGui.QAction('&Block compilations', None)
        self.blockCompilationsAction.setCheckable(True)
        self.blockCompilationsAction.setChecked(False)
        self.blockCompilationsAction.triggered.connect(self.onSetBlockCompilations)
  ...
  def onHotkeyPressed(self, key, modifiers, hotkey):
    if hotkey == DFG.DFGHotkeys.EXECUTE: self.onDirty()
    elif hotkey == DFG.DFGHotkeys.NEW_SCENE: self.onNewGraph()
    elif hotkey == DFG.DFGHotkeys.OPEN_SCENE: self.onLoadGraph()
    elif hotkey == DFG.DFGHotkeys.SAVE_SCENE: self.__saveGraph(False)
    elif hotkey == DFG.DFGHotkeys.TOGGLE_MANIPULATION:
      # Make sure we use the Action path, so menu's "checked" state is updated
      if self.manipAction: self.manipAction.trigger()
    else: self.dfgWidget.onHotkeyPressed(key, modifiers, hotkey)

  def onGraphSet(self, graph):
    if graph != self.currentGraph:
      graph = self.dfgWidget.getUIGraph()
      graph.defineHotkey(QtCore.Qt.Key_Delete, QtCore.Qt.NoModifier, DFG.DFGHotkeys.DELETE_1)
